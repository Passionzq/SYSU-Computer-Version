...
CImg<float> Hough::initHoughSpace() {
    ...
    hough_space.save("../result/hough.bmp");
	return hough_space;
}
// 投票算法找出霍夫空间中直线经过最多的点
...
void Hough::drawLines() {
	...
	for (int i = 0; i < lines.size(); i++) {
		...
		if (abs(lines[i].k) > 1) {
			result.draw_line(x0, y_min, x1, y_max, lines_color,10);
		}
		else {
			result.draw_line(x_min, y0, x_max, y1, lines_color,10);
		}
	}
	result.save("../result/line.bmp");
}
// 寻找并画出直线交点
